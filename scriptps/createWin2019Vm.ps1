...
if ( $null -eq $result) {
    ...
    $secureLocalAdministratorPassword = ConvertTo-SecureString -AsPlainText $LocalAdministratorPassword -Force

    New-BasicUnattendXML -ComputerName $name `
        -LocalAdministratorPassword $secureLocalAdministratorPassword `
